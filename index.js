...
if (fs.existsSync(DATA_FILE)) {
  try {
    ...
    if (!data.puzzlePoints) data.puzzlePoints = {};
...
const puzzles = [
  ...
  {
    question: "بيت ليس له أبواب ولا نوافذ، ما هو؟",
    answers: ["بيت الشعر", "الشعر"]
  },

  {
    question: "ما هو الشيء الذي إذا كسرته لا تستطيع إصلاحه؟",
    answers: ["الوعد", "الوعد"]
  },

  {
    question: "شيء موجود في القرن مرة وفي الدقيقة مرتين ولا يوجد في الساعة، ما هو؟",
    answers: ["حرف القاف", "القاف"]
  },
...

  {
    ...
    answers: ["الزجاجة", "زجاجة", "القارورة", "قارورة"]
  },

  {
    question: "ما هو الشيء الذي إذا مشى وقف وإذا وقف مشى؟",
    answers: ["الساعة", "الساعه"]
  },

  {
    question: "ما هو الشيء الذي يرى كل شيء وليس له عيون؟",
    answers: ["المرآة", "المراية", "المرأه", "مراية"]
  },
...
// اللغز الحالي في كل سيرفر
const activePuzzles = {};

// آخر لغز تم استخدامه في كل سيرفر
const lastPuzzle = {};
...
function isCorrectPuzzleAnswer(message, puzzle) {
  ...
  return puzzle.answers.some(
    correct =>
      normalizeAnswer(correct) === answer
  );

}

function getRandomPuzzle(guildId) {

  let available =
    puzzles.filter(
      (_, index) =>
        index !== lastPuzzle[guildId]
    );
  ...
  lastPuzzle[guildId] = index;

  return puzzle;

}

function addPuzzlePoints(userId, amount) {

  if (!data.puzzlePoints[userId]) {
    data.puzzlePoints[userId] = 0;
  }

  data.puzzlePoints[userId] += amount;

  if (data.puzzlePoints[userId] < 0) {
    data.puzzlePoints[userId] = 0;
  }
...
client.on(
  ...
  async message => {
    ...
    try {
      ...
      // =================================================
      // بدء لغز عند كتابة "لغز"
      // =================================================

      if (
        message.content.trim().toLowerCase() === "لغز"
      ) {

        // إذا يوجد لغز فعال
        if (
          activePuzzles[message.guild.id]
        ) {

          return message.reply(
            "⏳ يوجد لغز حاليًا، حاول حله أولًا!"
          );
        ...
        const embed =
          new EmbedBuilder()
            ...
            .setDescription(
              `❓ **${puzzle.question}**\n\n` +
              `⏱️ معك **دقيقة واحدة** للإجابة!\n` +
              `🏆 الإجابة الصحيحة = **10 نقاط ألغاز**`
            )
            ...
            .setFooter({
              text:
                "أول إجابة صحيحة تحصل على النقاط"
            })
        ...
        const embed =
          new EmbedBuilder()
            ...
            .setDescription(
              ...
              `🏆 حصلت على **10 نقاط ألغاز**.\n\n` +

              `⭐ **نقاط الألغاز لديك:** ${total}`
