...
client.on("guildMemberAdd", async member => {
  ...
  try {
    ...
    const visitorRole =
      member.guild.roles.cache.get(ROLES.VISITOR);

    if (!visitorRole) {
      console.log("❌ Visitor role not found");
      return;
    }

    if (!member.roles.cache.has(ROLES.VISITOR)) {
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === "warn") {
        ...
        const target =
          interaction.options.getMember("person");
        ...
        // لا يمكن للموظف الأقل تحذير موظف أعلى منه
